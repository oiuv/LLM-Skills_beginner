...
// 注册工具：get_weather（新 API：registerTool + Zod schema）
server.registerTool(
  "get_weather",
  {
    title: "天气查询",
...
server.registerTool(
  "get_forecast",
  {
    ...
    inputSchema: z.object({
      ...
      days: z.number().min(1).max(3).default(3).describe("预报天数（1-3天）"),
    }),
  },
  async ({ city, days }) => {
    const data = weatherDB[city];
...
server.registerTool(
  ...
  async () => {
    ...
    return {
      content: [{ type: "text" as const, text: `支持查询的城市（共${cities.length}个）：\n${cities.join("、")}` }],
    };
  }
);

// 辅助函数
function getWeatherAdvice(data: WeatherData): string {
